...
bool MatchParser::parse() {
  ...
  for (it = m_args.constBegin(); it != m_args.constEnd(); ++it) {
    ...
    if (!option.duplicates && contains(name)) {
      qWarning("Multiple instances of option \"%s\"", qUtf8Printable(name));
      return false;
    }

    if (list.size() < option.minArgs) {
      if (option.maxArgs == option.minArgs)
        qWarning("Option \"%s\" needs %d argument(s)", qUtf8Printable(name),
                 option.minArgs);
      else
    ...
    if (option.maxArgs != -1 && list.size() > option.maxArgs) {
      qWarning("Too many arguments for option \"%s\"", qUtf8Printable(name));
      return false;
    }
...
QMap<QString, QString> MatchParser::Option::toMap(
  ...
  for (const auto& arg : args) {
    ...
    if (argName.isEmpty() || argVal.isEmpty() || !defaults.contains(argName) ||
        map.contains(argName)) {
      qWarning("Invalid argument for option \"%s\": \"%s\"",
               qUtf8Printable(name), qUtf8Printable(arg));
  ...
  if (map.size() != defaults.size()) {
    ...
    if (!missing.isEmpty()) {
      qWarning("Option \"%s\" needs argument(s): %s", qUtf8Printable(name),
