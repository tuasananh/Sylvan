...
EngineMatch* parseMatch(const QStringList& args, QObject* parent) {
  ...
  for (const auto& option : options) {
    ...
    // Threshold for resign adjudication
      ...
      QMap<QString, QString> params =
          option.toMap("movecount|score|twosided=false");
      bool countOk = false;
      bool scoreOk = false;
      int moveCount = params["movecount"].toInt(&countOk);
      int score = params["score"].toInt(&scoreOk);
    ...
    // Use an opening suite
      ...
      QMap<QString, QString> params = option.toMap(
          "file|format=pgn|order=sequential|plies=1024|start=1|policy=default");
      ok = !params.isEmpty();
    ...
    if (!ok) {
      ...
      if (value.isValid() && value.type() == QVariant::Bool)
        qWarning("Empty value for option \"%s\"", qUtf8Printable(name));
      else {
        QString val;
  ...
  for (const auto& engine : constEngines) {
    ...
    tournament->addPlayer(new EngineBuilder(engine.config), engine.tc,
                          match->addOpeningBook(engine.book), engine.bookDepth);
  }
...
int main(int argc, char* argv[]) {
  ...
  for (const auto& arg : constArguments) {
    if (arg == "-v" || arg == "--version" || arg == "-version") {
      ...
      out << "Copyright (C) 2008-2018 Ilari Pihlajisto and Arto Jonsson"
          << endl;
