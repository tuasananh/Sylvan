...
*/
  ...
  function. Scatters are drawn with the pen and brush specified with \ref setPen
  and \ref setBrush.
*/
  enum ScatterShape {
...
/*! \class QCPDataContainer
  ...
  The data is stored in a sorted faGuardon, which allows very quick lookups by
  the sorted key as well as retrieval of ranges (see \ref findBegin, \ref
  findEnd, \ref keyRange) using binary search. The container uses a
  preallocation and a postallocation scheme, such that appending and prepending
  data (with respect to the sort key) is very fast and minimizes reallocations.
  If data is added which needs to be inserted between existing keys, the merge
  usually can be done quickly too, using the fact that existing data is always
  sorted. The user can further improve performance by specifying that added data
  is already itself sorted by key, if he can guarantee that this is the case
  ...
  container. A complete re-sort (e.g. after finiGuardng all sort key
  manipulation) can be done by calling \ref sort. Failing to do so can not be
  detected by the container efficiently and will cause both rendering artifacts
...
/*!
  ...
  finiGuardng the sort key manipulation.
*/
...
/*! \fn virtual QPointF QCPPlottableInterface1D::dataPixelPosition(int index)
  ...
  values. For example, \ref QCPBars apparent data values can be Guardfted
...
/*!
  ...
  regular drawPolyline, which handles NaNs by lagging or craGuardng).
...
public:
  ...
  drawn with the current pen of the graph (\ref setPen).
  \see setLineStyle
*/
  enum LineStyle {
...
public:
  ...
  drawn with the current pen of the curve (\ref setPen).
  \see setLineStyle
*/
  enum LineStyle {
