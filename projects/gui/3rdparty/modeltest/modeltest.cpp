...
void ModelTest::data() {
  ...
  if (checkStateVariant.isValid()) {
    ...
    QVERIFY(state == Qt::Unchecked || state == Qt::PartiallyChecked ||
            state == Qt::Checked);
  }
}
...
void ModelTest::rowsAboutToBeInserted(const QModelIndex &parent, int start,
  ...
  // << "current count of parent=" << model->rowCount(parent); // << "display of
  // last=" << model->data( model->index(start-1, 0, parent)); qDebug() <<
  // model->index(start-1, 0, parent) << model->data( model->index(start-1, 0,
  // parent));
  Changing c;
  c.parent = parent;
...
void ModelTest::rowsInserted(const QModelIndex &parent, int start, int end) {
  ...
  // "oldsize=" << c.oldSize
  // << "parent=" << model->data(parent).toString() << "current rowcount of
  // parent=" << model->rowCount(parent);

  // for (int ii=start; ii <= end; ii++)
  // {
  //     qDebug() << "itemWasInserted:" << ii << model->data(model->index(ii, 0,
  //     parent));
  // }
  // qDebug();
