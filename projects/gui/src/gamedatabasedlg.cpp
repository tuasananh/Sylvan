...
GameDatabaseDialog::GameDatabaseDialog(GameDatabaseManager *dbManager,
  ...
  connect(ui->m_importBtn, &QPushButton::clicked, this, [=]() {
    auto dlg =
        new QFileDialog(this, tr("Import Game"), QString(),
                        tr("Portable Game Notation (*.pgn);;All Files (*.*)"));
    connect(dlg, &QFileDialog::fileSelected, m_dbManager,
            &GameDatabaseManager::importPgnFile);
  ...
  connect(ui->m_advancedSearchBtn, SIGNAL(clicked()), this,
...
void GameDatabaseDialog::gameSelectionChanged(const QModelIndex &current,
  ...
  if ((status = selectedDatabase->game(entry, &m_game)) != PgnDatabase::Ok) {
    if (status == PgnDatabase::DoesNotExist) {
      ...
      msgBox.setInformativeText(tr("Remove %1 from the list of databases?")
                                    .arg(selectedDatabase->displayName()));
      ...
      if (status == PgnDatabase::Modified) {
        msgBox.setText(
            tr("PGN database has been modified since the last import."));
        msgBox.setInformativeText(
            tr("The database must be imported again to read it."));
      } else {
        msgBox.setText(
            tr("Error occured while trying to read the PGN database."));
        msgBox.setInformativeText(
            tr("Importing the database again may fix this problem."));
      }
