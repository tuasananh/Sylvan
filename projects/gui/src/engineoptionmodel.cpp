...
EngineOptionModel::EngineOptionModel(QList<EngineOption*> options,
                                     QObject* parent)
    : QAbstractItemModel(parent), m_options(options) {}
  ...
  if (role == Qt::DisplayRole) {
    switch (index.column()) {
      ...
      case 1:
        ...
        if (option->value().type() == QVariant::Bool) return QVariant();
...
Qt::ItemFlags EngineOptionModel::flags(const QModelIndex& index) const {
  ...
  if (index.column() == 1) {
    ...
    if (!option->isEditable()) return defaultFlags;
...
bool EngineOptionModel::setData(const QModelIndex& index, const QVariant& data,
  ...
  if (index.column() == 0) {
    ...
    if (stringData.isEmpty()) return false;
