...
EvalWidget::EvalWidget(QWidget* parent)
  ...
  statsHeaders << tr("Speed") << tr("Hash") << tr("Ponder") << tr("Probability")
               << tr("EGB");
...
void EvalWidget::onEval(const MoveEvaluation& eval) {
  ...
  if (nps) {
    QString npsStr =
        nps < 10000 ? QString("%1").arg(nps) : QString("%1k").arg(nps / 1000);
