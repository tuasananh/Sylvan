...
EngineConfigurationDialog::EngineConfigurationDialog(
  ...
  for (QStringList::iterator iter = protocols.begin(); iter != protocols.end();
       iter++)
  ...
  connect(ui->m_browseCmdBtn, SIGNAL(clicked(bool)), this,
          SLOT(browseCommand()));
  connect(ui->m_browseWorkingDirBtn, SIGNAL(clicked(bool)), this,
          SLOT(browseWorkingDir()));
  connect(ui->m_detectBtn, SIGNAL(clicked()), this,
          SLOT(detectEngineOptions()));
  connect(ui->m_restoreBtn, SIGNAL(clicked()), this, SLOT(restoreDefaults()));
  connect(ui->m_tabs, SIGNAL(currentChanged(int)), this,
          SLOT(onTabChanged(int)));
  connect(ui->m_nameEdit, SIGNAL(textChanged(QString)), this,
          SLOT(onNameOrCommandChanged()));
  connect(ui->m_commandEdit, SIGNAL(textChanged(QString)), this,
          SLOT(onNameOrCommandChanged()));
...
void EngineConfigurationDialog::browseCommand() {
  ...
  auto dlg =
      new QFileDialog(this, tr("Select Engine Executable"), defaultDir, filter);
  dlg->setAttribute(Qt::WA_DeleteOnClose);
  dlg->setAcceptMode(QFileDialog::AcceptOpen);

  connect(dlg, &QFileDialog::fileSelected, this,
          &EngineConfigurationDialog::setExecutable);
  dlg->open();
}
...
void EngineConfigurationDialog::browseWorkingDir() {
  auto dlg = new QFileDialog(this, tr("Select Engine Working Directory"),
...
void EngineConfigurationDialog::detectEngineOptions() {
  ...
  if (m_engine != nullptr) {
    connect(m_engine, SIGNAL(ready()), this, SLOT(onEngineReady()));
    connect(m_engine, SIGNAL(disconnected()), this, SLOT(onEngineQuit()));
    connect(m_engine, SIGNAL(destroyed()), this, SIGNAL(detectionFinished()));
  } else {
    m_hasError = true;
...
void EngineConfigurationDialog::onEngineQuit() {
  if (m_engine != nullptr) {
    ...
    if (m_engine->hasError()) {
      m_hasError = true;
      QMessageBox::critical(this, tr("Engine Error"), m_engine->errorString());
    }
...
void EngineConfigurationDialog::onNameOrCommandChanged() {
  bool ok =
      !ui->m_nameEdit->text().isEmpty() && !ui->m_commandEdit->text().isEmpty();
...
void EngineConfigurationDialog::onAccepted() {
  ...
  connect(this, SIGNAL(detectionFinished()), this, SLOT(onDetectionFinished()));
  detectEngineOptions();
}
