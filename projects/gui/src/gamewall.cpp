...
void GameWallWidget::setGame(ChessGame* game) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    connect(player, SIGNAL(nameChanged(QString)), m_clocks[i],
    ...
    connect(player, SIGNAL(startedThinking(int)), m_clocks[i],
            SLOT(start(int)));
    connect(player, SIGNAL(stoppedThinking()), m_clocks[i], SLOT(stop()));
  }
