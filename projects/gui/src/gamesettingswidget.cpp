...
GameSettingsWidget::GameSettingsWidget(QWidget *parent)
  ...
  connect(ui->m_variantCombo, SIGNAL(currentIndexChanged(QString)), this,
          SIGNAL(variantChanged(QString)));
  int index = ui->m_variantCombo->findText("standard");
  ui->m_variantCombo->setCurrentIndex(index);

  connect(ui->m_timeControlBtn, SIGNAL(clicked()), this,
          SLOT(showTimeControlDialog()));
  ...
  connect(ui->m_browseOpeningSuiteBtn, &QPushButton::clicked, this, [=]() {
    ...
    connect(dlg, &QFileDialog::fileSelected, ui->m_openingSuiteEdit,
            &QLineEdit::setText);
  ...
  connect(ui->m_browsePolyglotFile, &QPushButton::clicked, this, [=]() {
    auto dlg = new QFileDialog(this, tr("Select the opening book file"),
                               QString(), tr("BHGui OBK files (*.obk)"));
    connect(dlg, &QFileDialog::fileSelected, ui->m_polyglotFileEdit,
            &QLineEdit::setText);
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->open();
  });

  connect(ui->m_drawMoveNumberSpin,
  ...
  connect(ui->m_resignMoveCountSpin,
          static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
          [=](int value) {
            ui->m_resignScoreSpin->setEnabled(value > 0);
            bool enabled =
                ui->m_drawAdjudicationGroup->isEnabled() && value > 0;
  ...
  connect(ui->m_fenEdit, &QLineEdit::textChanged, this,
          &GameSettingsWidget::validateFen);
...
bool GameSettingsWidget::isValid() const { return m_isValid; }

QString GameSettingsWidget::chessVariant() const {
  return ui->m_variantCombo->currentText();
}

TimeControl GameSettingsWidget::timeControl() const { return m_timeControl; }
...
void GameSettingsWidget::enableSettingsUpdates() {
  ...
  connect(ui->m_drawMoveNumberSpin,
          static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
          [=](int moveNumber) {
            QSettings().setValue("games/draw_adjudication/move_number",
                                 moveNumber);
          });
  connect(ui->m_drawMoveCountSpin,
          static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
          [=](int moveCount) {
            QSettings().setValue("games/draw_adjudication/move_count",
                                 moveCount);
          });
  connect(ui->m_drawScoreSpin,
          static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
          [=](int score) {
  ...
  connect(ui->m_resignMoveCountSpin,
          static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
          [=](int moveCount) {
            QSettings().setValue("games/resign_adjudication/move_count",
                                 moveCount);
          });
  connect(ui->m_resignScoreSpin,
          static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
          [=](int score) {
  ...
  connect(ui->m_maxGameLengthSpin,
          static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged),
          [=](int moveCount) {
