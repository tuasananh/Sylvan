...
void ImportProgressDialog::updateImportStatus(const QTime& startTime,
  ...
  ui->m_importProgressBar->setValue(
      int((double(numReadBytes) / m_totalFileSize) * 100));

  int remainingSecs =
      (m_totalFileSize - numReadBytes) / (numReadBytes / elapsed);

  ui->m_statusLabel->setText(QString(tr("%1 games/sec - %2"))
                                 .arg((int)numReadGames / elapsed)
