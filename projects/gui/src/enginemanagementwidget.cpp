...
EngineManagementWidget::EngineManagementWidget(QWidget* parent)
  ...
  m_filteredModel->setSourceModel(
      new EngineConfigurationModel(m_engineManager, this));
  ...
  connect(ui->m_searchEngineEdit, SIGNAL(textChanged(QString)), this,
  ...
  connect(ui->m_browseDefaultLocationBtn, SIGNAL(clicked()), this,
          SLOT(browseDefaultLocation()));

  updateEngineCount();
}

EngineManagementWidget::~EngineManagementWidget() { delete ui; }

bool EngineManagementWidget::hasConfigChanged() const { return m_hasChanged; }
...
void EngineManagementWidget::addEngine() {
  auto dlg =
      new EngineConfigurationDialog(EngineConfigurationDialog::AddEngine, this);
...
void EngineManagementWidget::removeEngine() {
  const QItemSelection selection = m_filteredModel->mapSelectionToSource(
...
void EngineManagementWidget::browseDefaultLocation() {
  auto dlg = new QFileDialog(this, tr("Choose Directory"),
