...
NewTournamentDialog::NewTournamentDialog(EngineManager *engineManager,
  ...
  connect(ui->m_gameSettings, SIGNAL(variantChanged(QString)), this,
          SLOT(onVariantChanged(QString)));

  connect(ui->m_addEngineBtn, SIGNAL(clicked()), this, SLOT(addEngine()));
  connect(ui->m_removeEngineBtn, SIGNAL(clicked()), this, SLOT(removeEngine()));
  ...
  connect(ui->m_browsePgnoutBtn, &QPushButton::clicked, this, [=]() {
    ...
    connect(dlg, &QFileDialog::fileSelected, ui->m_pgnoutEdit,
            &QLineEdit::setText);
  ...
  connect(ui->m_browseEpdoutBtn, &QPushButton::clicked, this, [=]() {
    ...
    connect(dlg, &QFileDialog::fileSelected, ui->m_epdoutEdit,
            &QLineEdit::setText);
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->setAcceptMode(QFileDialog::AcceptSave);
    dlg->open();
  ...
  m_addedEnginesModel =
      new EngineConfigurationModel(m_addedEnginesManager, this);
  ...
  connect(ui->m_playersList, SIGNAL(doubleClicked(QModelIndex)), this,
...
void NewTournamentDialog::moveEngine(int offset) {
  ...
  m_addedEnginesManager->updateEngineAt(row1,
                                        m_addedEnginesManager->engineAt(row2));
...
void NewTournamentDialog::onPlayerSelectionChanged(
  ...
  ui->m_moveEngineDownBtn->setEnabled(
...
NewTournamentDialog::createTournament(GameManager *gameManager) const {
  ...
  auto tf =
      TournamentFactory::create(ts->tournamentType(), gameManager, parent());
  ...
  for (EngineConfiguration config : engines) {
    ...
    tf->addPlayer(new EngineBuilder(config), ui->m_gameSettings->timeControl(),
                  book, bookDepth);
  }
...
void NewTournamentDialog::readSettings() {
  ...
  if (pgnName.isEmpty()) {
    pgnName = QSettings()
                  .value("tournament/default_pgn_output_file", QString())
                  .toString();
  ...
  if (epdName.isEmpty()) {
    epdName = QSettings()
                  .value("tournament/default_epd_output_file", QString())
                  .toString();
