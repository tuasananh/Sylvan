...
GraphicsBoard::~GraphicsBoard() { delete m_highlightAnim; }

int GraphicsBoard::type() const { return Type; }

QRectF GraphicsBoard::boundingRect() const {
  const auto margins =
      QMarginsF(m_coordSize, m_coordSize, m_coordSize, m_coordSize);
...
void GraphicsBoard::paint(QPainter* painter,
  ...
  painter->drawLine(m_squareSize * 8 + x + off, y - off,
  ...
  for (int i = 1; i < 8; i++) {
    painter->drawLine(x + m_squareSize * i, y, x + m_squareSize * i,
                      y + m_squareSize * 4);
    painter->drawLine(x + m_squareSize * i, y + m_squareSize * 5,
                      x + m_squareSize * i, y + m_squareSize * 9);
  }

  painter->drawLine(x, y, x, y + m_squareSize * 9);
  painter->drawLine(x + m_squareSize * 8, y, x + m_squareSize * 8,
                    y + m_squareSize * 9);

  for (int i = 0; i <= 9; i++)
    painter->drawLine(x, y + m_squareSize * i, x + m_squareSize * 8,
                      y + m_squareSize * i);

  painter->drawLine(x + m_squareSize * 3, y, x + m_squareSize * 5,
                    y + m_squareSize * 2);
  painter->drawLine(x + m_squareSize * 5, y, x + m_squareSize * 3,
                    y + m_squareSize * 2);
  painter->drawLine(x + m_squareSize * 3, y + m_squareSize * 7,
                    x + m_squareSize * 5, y + m_squareSize * 9);
  painter->drawLine(x + m_squareSize * 5, y + m_squareSize * 7,
                    x + m_squareSize * 3, y + m_squareSize * 9);
  ...
  for (int j = 2; j > 0; j--) {
    for (int i = 1; i < 5; i++) {
      QPoint points1[3] = {
          QPoint(x + m_squareSize * (2 * i - 2) + r5,
                 y + m_squareSize * (9 - j * 3) - r15),
          QPoint(x + m_squareSize * (2 * i - 2) + r5,
                 y + m_squareSize * (9 - j * 3) - r5),
          QPoint(x + m_squareSize * (2 * i - 2) + r15,
                 y + m_squareSize * (9 - j * 3) - r5),
      };
      painter->drawPolyline(points1, 3);

      QPoint points2[3] = {
          QPoint(x + m_squareSize * (2 * i + 0) - r15,
                 y + m_squareSize * (9 - j * 3) - r5),
          QPoint(x + m_squareSize * (2 * i + 0) - r5,
                 y + m_squareSize * (9 - j * 3) - r5),
          QPoint(x + m_squareSize * (2 * i + 0) - r5,
                 y + m_squareSize * (9 - j * 3) - r15),
      };
      ...
      if (i < 3) {
        QPoint points5[3] = {
            QPoint(x + m_squareSize * pow(2, 2 * i - 1) - m_squareSize - r15,
                   y + m_squareSize * (pow(j + 1, 2) - 2) - r5),
            QPoint(x + m_squareSize * pow(2, 2 * i - 1) - m_squareSize - r5,
                   y + m_squareSize * (pow(j + 1, 2) - 2) - r5),
            QPoint(x + m_squareSize * pow(2, 2 * i - 1) - m_squareSize - r5,
                   y + m_squareSize * (pow(j + 1, 2) - 2) - r15),
        };
