...
PgnDatabase::Status PgnDatabase::status() const {
  ...
  if (!info.exists()) return DoesNotExist;
  if (!info.isReadable()) return Unreadable;
  if (info.lastModified() != m_lastModified) return Modified;
...
PgnDatabase::Status PgnDatabase::game(const PgnGameEntry* entry,
  ...
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) return Unreadable;
