...
QSqlDatabase DatabaseManager::database(const QString& connectionName,
  ...
  if (!thread->objectName().isEmpty()) {
    QString objectname =
        QString::number((long long)QThread::currentThread(), 16);
    // if we have a connection for this thread, return it
    QMap<QString, QMap<QString, QSqlDatabase>>::Iterator it_thread =
        s_instances.find(objectname);
    if (it_thread != s_instances.end()) {
      QMap<QString, QSqlDatabase>::iterator it_conn =
          it_thread.value().find(connectionName);
  ...
  */
  ...
  QSqlDatabase connection = QSqlDatabase::addDatabase(
      "QSQLITE", QString("%1_%2").arg(connectionName).arg(objectname));
  ...
  qDebug() << "Function Name: " << Q_FUNC_INFO
           << " new SQL connection instances Thread: "
           << thread->currentThread() << " Name: " << connectionName;

  s_instances[objectname][connectionName] = connection;
  qDebug() << "Function Name: " << Q_FUNC_INFO
           << " connection instances used: " << s_instances.size();

  return connection;
}

void DatabaseManager::clear() { s_instances.clear(); }
...
void DatabaseManager::removeCurrentThread(QString connectionName) {
  ...
  if (s_instances.contains(objectname)) {
    ...
    if (!connection.isOpen()) {
      ...
      qDebug() << "Function Name: " << Q_FUNC_INFO
               << " remove connection instance: " << objectname;
    }
  }
