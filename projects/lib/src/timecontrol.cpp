...
bool TimeControl::operator==(const TimeControl& other) const {
  if (m_movesPerTc == other.m_movesPerTc && m_timePerTc == other.m_timePerTc &&
      ...
      m_nodeLimit == other.m_nodeLimit && m_infinite == other.m_infinite)
    return true;
  return false;
}

bool TimeControl::isValid() const {
  if (m_movesPerTc < 0 || m_timePerTc < 0 || m_timePerMove < 0 ||
      m_increment < 0 || m_plyLimit < 0 || m_nodeLimit < 0 ||
      m_expiryMargin < 0 || (m_timePerTc == m_timePerMove && !m_infinite))
    return false;
  return true;
}
...
QString TimeControl::toVerboseString() const {
  ...
  if (m_infinite)
    ...
    str = tr("%1 per move").arg(s_timeString(m_timePerMove));
  else if (m_movesPerTc != 0)
    str = tr("%1 moves in %2").arg(m_movesPerTc).arg(s_timeString(m_timePerTc));
  else
...
bool TimeControl::isInfinite() const { return m_infinite; }

int TimeControl::timePerTc() const { return m_timePerTc; }

int TimeControl::movesPerTc() const { return m_movesPerTc; }

int TimeControl::timeIncrement() const { return m_increment; }

int TimeControl::timePerMove() const { return m_timePerMove; }

int TimeControl::timeLeft() const { return m_timeLeft; }

int TimeControl::movesLeft() const { return m_movesLeft; }

int TimeControl::plyLimit() const { return m_plyLimit; }

qint64 TimeControl::nodeLimit() const { return m_nodeLimit; }

int TimeControl::expiryMargin() const { return m_expiryMargin; }

void TimeControl::setInfinity(bool enabled) { m_infinite = enabled; }
...
void TimeControl::setTimeLeft(int timeLeft) { m_timeLeft = timeLeft; }
...
void TimeControl::startTimer() { m_time.start(); }
...
int TimeControl::lastMoveTime() const { return m_lastMoveTime; }
