...
bool EpdRecord::parse(QTextStream& stream) {
  enum TokenType { SeparatorToken, OpcodeToken, OperandToken };
    ...
    switch (c.toLatin1()) {
      ...
      case '\"':
        if (type == OpcodeToken) {
          ...
          type = OperandToken;
        } else if (type == OperandToken && !inQuotes && !operand.isEmpty()) {
          ok = false;
          break;
        }
        inQuotes = !inQuotes;
      ...
      case ';':
        ...
        break;
      default:
        if (type == SeparatorToken) type = OpcodeToken;
        if (type == OpcodeToken)
          opcode.append(c);
        else
          operand.append(c);
...
QString EpdRecord::fen() const { return m_fen; }
