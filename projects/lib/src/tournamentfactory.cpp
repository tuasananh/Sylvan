...
Tournament* TournamentFactory::create(const QString& type, GameManager* manager,
                                      QObject* parent) {
  if (type == "round-robin") return new RoundRobinTournament(manager, parent);
  if (type == "gauntlet") return new GauntletTournament(manager, parent);
  if (type == "knockout") return new KnockoutTournament(manager, parent);
  if (type == "pyramid") return new PyramidTournament(manager, parent);
