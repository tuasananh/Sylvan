...
int EngineProcess::exitCode() const { return (int)m_exitCode; }
...
QString EngineProcess::cmdLine(const QString& wdir, const QString& prog,
  ...
  if (!QFile::exists(cmd) && !wdir.isEmpty()) {
    ...
    if (!QFile::exists(cmd) && !prog.startsWith('\"') && !args.isEmpty()) {
...
HANDLE EngineProcess::mainJob() {
  ...
  SetInformationJobObject(s_job, JobObjectExtendedLimitInformation, &jeli,
                          sizeof(jeli));
