...
ChessPlayer* EngineBuilder::create(QObject* receiver, const char* method,
  ...
  if (!EngineFactory::protocols().contains(m_config.protocol())) {
    setError(error, tr("Unknown chess protocol: %1").arg(m_config.protocol()));
    return nullptr;
  }
  ...
  if (!ok) {
    setError(error, tr("Cannot execute command: %1").arg(m_config.command()));
    delete process;
    return nullptr;
  }
  ...
  if (receiver != nullptr && method != nullptr)
    QObject::connect(engine, SIGNAL(debugMessage(QString)), receiver, method);
