...
QString variantFromUci(QString str, bool uciPrefix = true) {
  if (uciPrefix) {
    if (!str.startsWith("UCI_")) return QString();
    str = str.mid(4);
  }
...
QString variantToUci(const QString& str, bool uciPrefix = true) {
  ...
  if (str == "fischerandom") return uciPrefix ? "UCI_Chess960" : "chess960";
  if (!uciPrefix) return str;
  if (str == "caparandom") return "UCI_CapaRandom";
...
void UciEngine::startGame() {
  ...
  if (m_sendOpponentsName) {
    ...
    QString value = QString("none none %1 %2").arg(opType, opponent()->name());
...
void UciEngine::makeMove(const Chess::Move& move) {
  ...
  if (m_ponderState != PonderHit) {
    m_ponderState = NotPondering;
    m_moveStrings +=
        " " + board()->moveString(move, Chess::Board::LongAlgebraic);
    if (m_ignoreThinking) {
...
bool UciEngine::isPondering() const { return (m_ponderState != NotPondering); }

void UciEngine::sendStop() { write("stop"); }

QString UciEngine::protocol() const { return "uci"; }
...
void UciEngine::sendQuit() { write("quit"); }
...
void UciEngine::parseInfo(const QVarLengthArray<QStringRef>& tokens, int type,
                          MoveEvaluation* eval) {
  enum Keyword {
    InfoDepth,
    InfoSelDepth,
...
EngineOption* UciEngine::parseOption(const QStringRef& line) {
  ...
  static const QString types[] = {"name", "type", "default",
                                  "min",  "max",  "var"};
...
void UciEngine::parseLine(const QString& line) {
  ...
  if (command == "info") {
    ...
    if (nextToken(command) == "error") {
      qWarning("Failed to register UCI engine %s", qUtf8Printable(name()));
    ...
    if (option == nullptr || !option->isValid())
      qWarning("Invalid UCI option from %s: %s", qUtf8Printable(name()),
               qUtf8Printable(line));
...
void UciEngine::setPonderMove(const QString& moveString) {
  ...
  if (m_ponderMove.isNull()) {
    m_ponderMoveSan.clear();
    qWarning("Illegal ponder move from %s: %s", qUtf8Printable(name()),
...
QString UciEngine::sanPv(const QVarLengthArray<QStringRef>& tokens) {
  ...
  for (auto token : tokens) {
    ...
    if (move.isNull()) {
      ...
      qWarning("Illegal PV move %s from %s (%d)", qUtf8Printable(tokenString),
               qUtf8Printable(name()), id());
