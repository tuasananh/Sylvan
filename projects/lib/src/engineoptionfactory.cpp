...
EngineOption *EngineOptionFactory::create(const QVariantMap &map) {
  ...
  if (value.type() != QVariant::Bool && value.type() != QVariant::String &&
    ...
    qWarning("Invalid value type for option: %s", qUtf8Printable(name));
    return nullptr;
  }
  ...
  if (defaultValue.isNull())
    ...
    qWarning("Invalid default value type for option: %s", qUtf8Printable(name));
    return nullptr;
  }
  ...
  if (type.isEmpty()) {
    return new EngineTextOption(name, value.toString(), defaultValue.toString(),
                                alias);
    ...
    return new EngineCheckOption(name, value.toBool(), defaultValue.toBool(),
                                 alias);
  } else if (type == "combo") {
