...
QString KnockoutTournament::type() const { return "knockout"; }

bool KnockoutTournament::canSetRoundMultiplier() const { return false; }
...
QList<int> KnockoutTournament::firstRoundPlayers() const {
  ...
  for (int i = 0; i < seedCount; i++) players << i;
...
void KnockoutTournament::initializePairing() {
  ...
  for (int i = 0; i < x; i += 2) pairs.append(pair(all.at(i), all.at(i + 1)));
...
QList<int> KnockoutTournament::lastRoundWinners() const {
  ...
  for (const TournamentPair* pair : last) winners << pair->leader();
...
bool KnockoutTournament::areAllGamesFinished() const {
  ...
  if (lastRound.size() > 1) return false;
...
TournamentPair* KnockoutTournament::nextPair(int gameNumber) {
  ...
  for (TournamentPair* pair : last) {
    if (needMoreGames(pair)) return pair;
  }

  QList<int> winners(lastRoundWinners());
  if (winners.size() <= 1 || gamesInProgress()) return nullptr;
...
QString KnockoutTournament::results() const {
  ...
  for (const TournamentPair* pair : first) {
    ...
    if (!pair->hasOriginalOrder()) std::swap(player1, player2);
  ...
  for (int round = 0; round < currentRound(); round++) {
    ...
    for (const TournamentPair* pair : nthRound) {
      ...
      QString text = QString("%1%2").arg(QString(r * 2, '\t'), winner);
      if (pair->scoreSum()) {
        int score1 = pair->firstScore();
        int score2 = pair->secondScore();
        if (!pair->hasOriginalOrder()) std::swap(score1, score2);

        text +=
            QString(" (%1-%2)").arg(double(score1) / 2).arg(double(score2) / 2);
      }
      lines[lineNum] += text;
      x++;
    }
