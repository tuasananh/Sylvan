...
ChessEngine::ChessEngine(QObject* parent)
  ...
  connect(m_protocolStartTimer, SIGNAL(timeout()), this,
          SLOT(onProtocolStartTimeout()));
}
...
void ChessEngine::setOption(const QString& name, const QVariant& value) {
  ...
  if (!option->isValid(value)) {
    qWarning("Invalid value for option %s: %s", qUtf8Printable(name),
...
bool ChessEngine::isPondering() const { return false; }

bool ChessEngine::redEvalPov() const { return m_redEvalPov; }
...
int ChessEngine::id() const { return m_id; }
...
void ChessEngine::onTimeout() { stopThinking(); }

void ChessEngine::ping(bool sendCommand) {
  if (m_pinging || state() == NotStarted || state() == Disconnected ||
...
void ChessEngine::write(const QString& data, WriteMode mode) {
  ...
  emit debugMessage(QString(">%1(%2): %3").arg(name()).arg(m_id).arg(data));

  if (m_ioDevice->write(data.toLatin1() + "\n") == -1)
    qWarning("Writing to engine %s(%d) failed", qUtf8Printable(name()), m_id);
}
...
void ChessEngine::quit() {
  ...
  disconnect(m_ioDevice, SIGNAL(readChannelFinished()), this,
             SLOT(onCrashed()));
  connect(m_ioDevice, SIGNAL(readChannelFinished()), this,
          SLOT(onQuitTimeout()));
  sendQuit();
  m_quitTimer->start();
}
