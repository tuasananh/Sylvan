...
bool MoveEvaluation::operator==(const MoveEvaluation &other) const {
  if (m_isBookEval == other.m_isBookEval && m_isTrusted == other.m_isTrusted &&
      ...
      m_pvNumber == other.m_pvNumber && m_hashUsage == other.m_hashUsage &&
      m_ponderhitRate == other.m_ponderhitRate &&
      m_nodeCount == other.m_nodeCount && m_nps == other.m_nps &&
      m_tbHits == other.m_tbHits && m_ponderMove == other.m_ponderMove)
    return true;
  return false;
}
...
bool MoveEvaluation::isEmpty() const {
  if (m_depth == 0 && m_selDepth == 0 && m_score == NULL_SCORE &&
      m_time < 500 && m_pvNumber == 0 && m_hashUsage == 0 &&
      m_ponderhitRate == 0 && m_nodeCount == 0 && m_nps == 0 && m_tbHits == 0 &&
...
bool MoveEvaluation::isBookEval() const { return m_isBookEval; }

bool MoveEvaluation::isTrusted() const { return m_isTrusted; }

int MoveEvaluation::depth() const { return m_depth; }

int MoveEvaluation::selectiveDepth() const { return m_selDepth; }
...
int MoveEvaluation::time() const { return m_time; }
...
quint64 MoveEvaluation::tbHits() const { return m_tbHits; }

int MoveEvaluation::hashUsage() const { return m_hashUsage; }

int MoveEvaluation::ponderhitRate() const { return m_ponderhitRate; }

QString MoveEvaluation::ponderMove() const { return m_ponderMove; }

QString MoveEvaluation::pv() const { return m_pv; }

int MoveEvaluation::pvNumber() const { return m_pvNumber; }
...
void MoveEvaluation::setBookEval(bool isBookEval) { m_isBookEval = isBookEval; }

void MoveEvaluation::setIsTrusted(bool isTrusted) { m_isTrusted = isTrusted; }

void MoveEvaluation::setDepth(int depth) { m_depth = depth; }

void MoveEvaluation::setSelectiveDepth(int depth) { m_selDepth = depth; }

void MoveEvaluation::setScore(int score) { m_score = score; }

void MoveEvaluation::setTime(int time) { m_time = time; }

void MoveEvaluation::setNodeCount(quint64 nodeCount) {
  m_nodeCount = nodeCount;
}

void MoveEvaluation::setNps(quint64 nps) { m_nps = nps; }

void MoveEvaluation::setTbHits(quint64 tbHits) { m_tbHits = tbHits; }

void MoveEvaluation::setHashUsage(int hashUsage) { m_hashUsage = hashUsage; }
