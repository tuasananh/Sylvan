...
bool Board::variantHasWallSquares() const { return false; }

QList<Piece> Board::reservePieceTypes() const { return QList<Piece>(); }
...
int Board::reserveType(int pieceType) const { return pieceType; }
...
GenericMove Board::genericMove(const Move& move) const {
  ...
  return GenericMove(chessSquare(source), chessSquare(target));
}
...
QString Board::fenString(FenNotation notation) const {
  ...
  for (int y = 0; y < m_height; y++) {
    ...
    for (int x = 0; x < m_width; x++) {
      ...
      // to the FEN string.
      if (nempty > 0 && (!pc.isEmpty() || x == m_width - 1)) {
