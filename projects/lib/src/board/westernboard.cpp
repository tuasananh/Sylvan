...
int WesternBoard::width() const { return 9; }

int WesternBoard::height() const { return 10; }
...
bool WesternBoard::vSetFenString(const QStringList &fen) {
  ...
  if (!kingsCountAssertion(kingCount[Side::Red], kingCount[Side::Black]))
    return false;
...
int WesternBoard::reversibleMoveCount() const { return m_reversibleMoveCount; }
