...
bool PgnGame::parseMove(PgnStream &in, bool addEco) {
  ...
  MoveData md = {board->key(), board->genericMove(move), str, QString()};
  addMove(md, addEco);
...
QString PgnGame::event() const { return m_tags.value("Event"); }

QString PgnGame::site() const { return m_tags.value("Site"); }

QDate PgnGame::date() const {
  return QDate::fromString(m_tags.value("Date"), "yyyy.MM.dd");
}

int PgnGame::round() const { return m_tags.value("Round").toInt(); }
...
Chess::Side PgnGame::startingSide() const { return m_startingSide; }
...
void PgnGame::setTag(const QString &tag, const QString &value) {
  ...
  if (m_tagReceiver)
    QMetaObject::invokeMethod(m_tagReceiver, "setTag", Qt::QueuedConnection,
...
// ChessGame objects are only deleted when they're no longer available in the
// GUI.
