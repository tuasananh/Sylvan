...
double BayesElo::bayesElo() const { return m_bayesElo; }

double BayesElo::drawElo() const { return m_drawElo; }
...
bool SprtProbability::isValid() const {
  return 0.0 < m_pWin && m_pWin < 1.0 && 0.0 < m_pLoss && m_pLoss < 1.0 &&
         0.0 < m_pDraw && m_pDraw < 1.0;
}

double SprtProbability::pWin() const { return m_pWin; }

double SprtProbability::pLoss() const { return m_pLoss; }
