...
TournamentPair::TournamentPair(int firstPlayer, int secondPlayer)
    : m_gamesStarted(0), m_hasOriginalOrder(true) {
...
bool TournamentPair::hasOriginalOrder() const { return m_hasOriginalOrder; }

int TournamentPair::firstPlayer() const { return m_first.index; }
...
int TournamentPair::gamesStarted() const { return m_gamesStarted; }

int TournamentPair::gamesFinished() const { return scoreSum() / 2; }

int TournamentPair::gamesInProgress() const {
  return m_gamesStarted - gamesFinished();
}

int TournamentPair::firstScore() const { return m_first.score; }

int TournamentPair::secondScore() const { return m_second.score; }

int TournamentPair::scoreSum() const { return m_first.score + m_second.score; }

int TournamentPair::scoreDiff() const { return m_first.score - m_second.score; }

void TournamentPair::addFirstScore(int score) { m_first.score += score; }

void TournamentPair::addSecondScore(int score) { m_second.score += score; }

void TournamentPair::addStartedGame() { m_gamesStarted++; }
