...
OpeningSuite::Format OpeningSuite::format() const { return m_format; }

OpeningSuite::Order OpeningSuite::order() const { return m_order; }
...
bool OpeningSuite::initialize() {
  ...
  if (!m_file->open(QIODevice::ReadOnly | QIODevice::Text)) {
    qWarning("Can't open opening suite %s", qUtf8Printable(m_fileName));
    delete m_file;
    return false;
  }
...
PgnGame OpeningSuite::nextGame(int maxPlies) {
  ...
  if (m_format == EpdFormat) {
    ...
    if (m_order == SequentialOrder && !ok && m_gamesRead > 0 &&
        m_epdStream->atEnd()) {
      m_epdStream->seek(0);
      m_epdStream->resetStatus();
      ok = epd.parse(*m_epdStream);
