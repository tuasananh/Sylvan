...
bool UcciEngine::isPondering() const { return (m_ponderState != NotPondering); }

void UcciEngine::sendStop() { write("stop"); }

QString UcciEngine::protocol() const { return "ucci"; }
...
EngineOption* UcciEngine::parseOption(const QStringRef& line) {
  ...
  static const QString types[] = {"option", "type", "default",
                                  "min",    "max",  "var"};
...
void UcciEngine::parseLine(const QString& line) {
  ...
  if (command == "info") {
    ...
    if (nextToken(command) == "error") {
      qWarning("Failed to register UCCI engine %s", qUtf8Printable(name()));
    ...
    if (option == nullptr || !option->isValid())
      qWarning("Invalid UCCI option from %s: %s", qUtf8Printable(name()),
               qUtf8Printable(line));
