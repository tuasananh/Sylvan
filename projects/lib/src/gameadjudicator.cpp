...
void GameAdjudicator::setDrawThreshold(int moveNumber, int moveCount,
                                       int score) {
...
void GameAdjudicator::addEval(const Chess::Board* board,
  ...
  if (m_drawMoveNum > 0) {
    ...
    if (board->plyCount() / 2 >= m_drawMoveNum &&
      ...
      m_result =
          Chess::Result(Chess::Result::Adjudication, Chess::Side::NoSide);
      return;
    }
  }
  ...
  if (m_resignMoveCount > 0) {
    ...
    if (count >= m_resignMoveCount &&
        (!m_twoSided ||
         m_winScoreCount[side.opposite()] >= m_resignMoveCount)) {
      m_result = Chess::Result(Chess::Result::Adjudication, side.opposite());
      return;
    }
  }

  // Trust-feature
  if (eval.isTrusted() && eval.score() >= eval.MATE_SCORE - 200) {
    m_result = Chess::Result(Chess::Result::Adjudication, side);
    return;
  }

  // Limit game length
  if (m_maxGameLength > 0 && board->plyCount() >= 2 * m_maxGameLength) {
    m_result = Chess::Result(Chess::Result::Adjudication, Chess::Side::NoSide);
    return;
  }
}

void GameAdjudicator::resetDrawMoveCount() { m_drawScoreCount = 0; }

Chess::Result GameAdjudicator::result() const { return m_result; }
